...
interface ProfileFormProps {
  userId: string;
  initialProfile: { display_name: string; avatar_url: string | null };
  initialOnboarding: {
    age: number | null;
    sex: string;
...
export function ProfileForm({ userId, initialProfile, initialOnboarding }: ProfileFormProps) {
  const router = useRouter();
  const supabase = createClient();

  const { unit_pref, height_cm, weight_kg } = initialOnboarding;
  const initUnit: Unit = unit_pref ?? "imperial";

  let initHeightFt = "", initHeightIn = "", initHeightCm = "", initWeight = "";
  if (initUnit === "metric") {
    initHeightCm = height_cm?.toString() ?? "";
    initWeight = weight_kg?.toString() ?? "";
  } else {
    if (height_cm) {
      const totalIn = height_cm / 2.54;
      initHeightFt = Math.floor(totalIn / 12).toString();
      initHeightIn = Math.round(totalIn % 12).toString();
    }
    initWeight = weight_kg ? Math.round(weight_kg / 0.453592).toString() : "";
  }

  const [unit, setUnit] = useState<Unit>(initUnit);
  const [age, setAge] = useState(initialOnboarding.age?.toString() ?? "");
  const [heightFt, setHeightFt] = useState(initHeightFt);
  const [heightIn, setHeightIn] = useState(initHeightIn);
  const [heightCm, setHeightCm] = useState(initHeightCm);
  const [weight, setWeight] = useState(initWeight);
  const [sex, setSex] = useState(initialOnboarding.sex ?? "");
