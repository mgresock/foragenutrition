...
interface GoalsFormProps {
  userId: string;
  initialGoals: string[];
  initialMealsPerWeek: number | null;
}

export function GoalsForm({ userId, initialGoals, initialMealsPerWeek }: GoalsFormProps) {
  const router = useRouter();
  const supabase = createClient();
  const [selected, setSelected] = useState<string[]>(initialGoals);
  const [mealsPerWeek, setMealsPerWeek] = useState<number | null>(initialMealsPerWeek);
