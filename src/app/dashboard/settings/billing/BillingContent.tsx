...
export function BillingContent({ tier: initialTier, aiUsed: initialAiUsed }: BillingContentProps) {
  const searchParams = useSearchParams();
  const [tier] = useState<"free" | "pro">(initialTier);
  const [aiUsed] = useState(initialAiUsed);
        ...
        <div className="mb-6 px-4 py-3 bg-card border border-border rounded-xl">
          ...
          <div className="h-1.5 bg-canvas rounded-full overflow-hidden">
            <div className="h-full rounded-full transition-all"
              style={{ width: `${aiPct}%`, background: aiPct >= 100 ? "#ef4444" : aiPct >= 80 ? "#FF9F0A" : "#2f9e44" }} />
          </div>
          {aiRemaining === 0 && (
